...
router.post('/analyze', auth, async (req, res) => {
  try {
    ...
    if (cycles.length < 2) {
      return res.json({
        message: 'Not enough data for AI analysis. Please log at least 2 cycles.',
        hasData: false
      });
    }
    ...
    try {
      ...
      const insight = await AIInsight.create({
        ...
        cycleData: response.data.cycleData,
        shouldDisplay: response.data.shouldDisplay || false,
        displayPriority: response.data.displayPriority || 0
      });
      ...
      res.status(503).json({
        ...
        message: 'Using baseline predictions instead'
      });
    }
  } catch (error) {
